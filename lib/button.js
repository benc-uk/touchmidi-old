...
Button.prototype.eventMouseDown = function(event) {
  ...
  if(this.widget.type == 'TOGGLE') {
    ...
    if(this.widget.toggled) {
      this.widget.triggerPressedActions(this.widget.midiactions);
    } else {
      this.widget.triggerUnpressedActions(this.widget.midiactions);
    }
  } else {
    this.widget.triggerPressedActions(this.widget.midiactions);
  }
...
Button.prototype.callMouseUp = function() {
  ...
  if(this.type != 'TOGGLE')
      this.triggerUnpressedActions(this.midiactions);
}
...
//
// =============================================================================
Button.prototype.triggerPressedActions = function(midiactions) {
  for (var i = 0; i < midiactions.length; i++) {
    var action = midiactions[i];
    if(action.type == 'note') {
...
//
// =============================================================================
Button.prototype.triggerUnpressedActions = function(midiactions) {
  for (var i = 0; i < midiactions.length; i++) {
    var action = midiactions[i];
    if(action.type == 'note') {
