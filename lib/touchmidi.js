...
function parseMIDIActions(node, widget) {
  ...
  if(node.hasAttribute('midicc')) {
    ...
    for (var i = 0; i < action_list.length; i++) {
      ...
      if(widget.constructor.name == 'Button') {
        action = new MidiAction('cc', parseInt(params[0].trim()));    //, parseInt(params[1].trim()), parseInt(params[2].trim()), parseInt(params[3].trim()) );
        action.cc = parseInt(params[1].trim());
        action.cc_val_on  = parseInt(params[2].trim());
        action.cc_val_off = parseInt(params[3].trim());
      } else {
        action = new MidiAction('cc', parseInt(params[0].trim()));     //, parseInt(params[1].trim()) );
        action.cc = parseInt(params[1].trim());
      }
      widget.midiactions.push(action);
    }
  ...
  if(node.hasAttribute('midinrpn')) {
    ...
    for (var i = 0; i < action_list.length; i++) {
      ...
      if(widget.constructor.name == 'Button') {
       //stubbed for now
        action = MidiAction('nrpn', parseInt(params[0].trim())); //, parseInt(params[1].trim()), parseInt(params[2].trim()), parseInt(params[3].trim()) );

      } else {
        // Special voodoo to overdide the max setting from 127 to whatever is specified as NRPN may have much higher ranges
        widget.max = parseInt(params[3].trim())
        action = new MidiAction('nrpn', parseInt(params[0].trim())); //, parseInt(params[1].trim()), parseInt(params[2].trim()) );
  ...
  if(node.hasAttribute('midiprog')) {
    ...
    for (var i = 0; i < action_list.length; i++) {
      var params = action_list[i].split(',');
      var action = null;
      var action = new MidiAction( 'prog', parseInt(params[0].trim()), parseInt(params[1].trim()), parseInt(params[2].trim()), parseInt(params[3].trim()) );
      widget.midiactions.push(action);
    }
  }
}
